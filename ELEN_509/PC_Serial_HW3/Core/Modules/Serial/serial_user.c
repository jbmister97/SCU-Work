...
uint8_t ProcessPacket(void)
{
  ...
  case 'V':
    msgLength = sprintf(msg2Send, "$v%c\n", buttonPushed == true ? 'P' : 'N');
    SendString(msg2Send, msgLength, StripZeros, NoAddCRLF);
    buttonPushed = false;
    break;
...
{
  // Eventually do something with the received byte
  // for now, just keep incrementing the pointer
